...
//        return resultList;
    ...
    public static List<Permission> buildPermission(List<Permission> permissionList){
        ...
        for (Permission currentPermission : permissionList) {
            // 0 是最上级的菜单，也是递归的入口
            if ("0".equals(currentPermission.getPid())){
                currentPermission.setLevel(1);
                finalPermissionList.add(selectChildren(currentPermission,permissionList));
                break;
            }
        }
    ...
    //===============================查询所有菜单，递归，返回树形结构====================================
    public static Permission selectChildren(Permission currentPermission, List<Permission> permissionList){
        // 初始化 children
        currentPermission.setChildren(new ArrayList<Permission>());
        for (Permission permission : permissionList) {
            // currentPermission 是上级菜单
            if (currentPermission.getId().equals(permission.getPid())){
                permission.setLevel(currentPermission.getLevel()+1);
                currentPermission.getChildren().add(selectChildren(permission,permissionList));
            }
        }
